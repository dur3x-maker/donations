...
function DonationRow({ donation, isNew }: { donation: RecentDonation; isNew: boolean }) {
  ...
  return (
    <div
      className={`rounded-2xl border border-stone-200 bg-white p-4 shadow-sm transition duration-500 ${
        isNew ? "opacity-100 ring-4 ring-emerald-100" : "opacity-95"
      }`}
    >
      <div className="flex items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-3">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-emerald-50 text-sm font-semibold text-emerald-800 ring-1 ring-emerald-100">
